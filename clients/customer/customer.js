import { isAliveGetProduct } from "./isAlive.js";
...
(async function () {
  ...
  const handleCountCartItems = () => {
    let count = 0;
    const countElements = document.querySelectorAll("[data-products-qtd]");
  ...
  async function handleAddToCart(event) {
    ...
    if (JSON.parse(getCart)) {
      const cartParsed = JSON.parse(getCart);
      const currentProductsInCart = cartParsed.products;
      const currentTotal = cartParsed.total;

      currentProductsInCart.push(parsedProduct);

      cart.total = Number(currentTotal) + Number(data.price);

      cart.products = currentProductsInCart;
    } else {
      cart.products = [parsedProduct];
  ...
  const createProductItem = (product) => {
    ...
    return `
            <div class="product">
                ...
                <div class="description"><p>Categoria: ${product.category}</p></div>
